...
public class CountUtilizationRate {
    ...
    public void checkAgvStatus() {
        boolean isPoweredOn = true; // 檢查AGV是否開機
...
//        boolean isWorking = checkStatus(); // 檢查AGV是否工作
        ...
        String week = currentDate.getDayOfWeek().toString();
        ...
        for(int i=0;i<3;i++){
            ...
            if (isPoweredOn) {
                // AGV開機處理邏輯
                analysisDao.updateOpenHours(analysis.getOpenHours()+1, analysisIds.get(i).getAnalysisId());
//                未處理資料庫問題，資料庫應改為分鐘
            } else {
                // AGV未開機處理邏輯
            }
